...
from gstatus.functions.network import isIP, hostAliasList, getIPv4Addresses
...
class Cluster:
    ...
    def getVersion(self):
        ...
        if os.path.exists('/etc/redhat-storage-release'):
            with open('/etc/redhat-storage-release', 'r') as RHS_version:
                ...
                lc_name = self.product_name.lower().replace('Update', 'u')
                self.product_shortname = "RHSS v%s" % (''.join(lc_name.split()[4:]))
        else:
                        ...
                        for node in node_elements:
                            ...
                            if node.find('./hostname').text == 'Self-heal Daemon':
                                ...
                                if not uuid:
                                    ...
                                    print "Cluster.updateState : Attempting to use a 'path' (%s) for a self heal daemon that" % (
                                        node_name)
    ...
    def calcConnections(self):
        ...
        for volume_xml in volumes:
            ...
            self.connection_count += self.volume[vol_name].connection_count
